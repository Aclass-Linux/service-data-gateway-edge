...
| 步骤 | nanoMODBUS | 我们 |
|------|-----------|------|
| 实例分配 | app 提供栈/静态内存 | `egw_modbus_server_create()` 内部 calloc |
...
| parser 创建 | create 内部初始化内部 ring buf | create 内部 `egw_proto_modbus_open(dir=REQUEST)` |
| 单次分配 | nmbs_t + 三个 buf 都在栈上（嵌入式风格） | `struct egw_modbus_server` 一次 calloc 含 parser |
...
dispatch:                                ← 仍在 nmbs_poll() 内部，引擎层
  ...
  └─ 匹配 unit_id:
        │
        ├─ unit_id == 0 → 广播帧，无条件处理（写请求）
        ├─ unit_id == t->unit_id → 命中
        └─ unit_id != t->unit_id → 丢弃
```
...
pdu[0] = fc ∈ {0x01, 0x02, 0x03, 0x04}
  ...
  │
  └─ 封装响应:
        resp_frame = wrap_pdu(resp_pdu, unit_id)
...
| create 内部分配三个 buf | create 内部 `egw_proto_modbus_open` 分配 parser |
| `nmbs_poll()` 内嵌 recv | App 层 `uv_poll_cb → egw_transport_read` |
| 内部 ring buf + parser | `egw_proto_handle_t`（reserve/commit） |
| `parse_request` + `unwrap_frame` | `egw_modbus_unwrap_frame` + `egw_modbus_parse_request` |
| `callbacks.read_registers` 回调 | `egw_modbus_srv_read_cb` 回调 |
| 内部 reg_buf 做字节序转换 | `regs[256]` 栈缓冲区 |
| `wrap_pdu` 封装响应 | `egw_modbus_wrap_pdu` + `egw_modbus_build_read_resp_pdu` |
| `has_response + platform.send` | `egw_modbus_server_get_response` + `egw_transport_write` |

**核心差异**：nanoMODBUS 把数据读写（回调）和字节序转换都放在同一 tick 里，我们也一样——`server_handle_frame` 里调回调 → 取 regs → `build_read_resp_pdu`（内部做 CPU 序 → 大端）→ `wrap_pdu`。
