...
├── protocol/    帧定界(egw_protocol) + Modbus RTU/TCP(egw_modbus_core/client/server)
...
**运行时数据流**（理解系统的关键）：
```
uv_poll fd 就绪 → egw_transport_read（读原始字节，直入 modbus reserve 缓冲区）
  → egw_modbus_server_commit（帧定界 + CRC 校验，零拷贝）
...
- **Protocol**：帧定界 + CRC，reserve/commit 零拷贝路径
- **Modbus**：`egw_modbus_core.c`（PDU 构建/encode/decode）+ `client.c`（主站状态机）+ `server.c`（从站状态机），RTU/TCP 双传输。输运通过 transport 枚举 + `egw_modbus_encode/decode` 统一分发
...
```c
/* transport 直接读入 server/client 的 reserve 缓冲区，省掉一次 memcpy */
size_t avail = 0;
uint8_t *wp = egw_modbus_server_reserve(server, &avail);
egw_transport_read(h, wp, &rlen, avail);           /* 直读 ring buf */
egw_modbus_server_commit(server, rlen);            /* 无 memcpy，触发帧定界 */
```
...
6. `src/protocol/egw_modbus_core.h` — Modbus 核心类型、PDU 构建、encode/decode
7. `src/protocol/egw_modbus_client.h` — Client（主站）API
8. `src/protocol/egw_modbus_server.h` — Server（从站）API
...
src/protocol/
├── include/
│   ├── egw_modbus_core.h     核心类型：常量、PDU 构建/解析、encode/decode
│   ├── egw_modbus_client.h   Client（主站）API
│   └── egw_modbus_server.h   Server（从站）API
├── egw_modbus_core.c         PDU 构建/解析 + encode_frame + 输运分发
├── egw_modbus_client.c       主站状态机（slot 环形链表，显式 slot 选择）
└── egw_modbus_server.c       从站状态机（真环形缓冲区，sending 背压）
```
