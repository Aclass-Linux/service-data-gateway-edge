...
| `core` | 基础组件（日志、配置、错误码） | 被以上各层依赖 | 🟡 部分实现 |
...
- **当前实现**：`src/app/main.c` — 加载 config.json（支持 -c 参数），打印 MQTT/Modbus 配置
- **M1 目标**：集成 MQTT 上传、看门狗、守护进程
...
- **当前实现**：配置框架（cJSON key-path 查询）、错误码定义（egw_err_t）、模块自动初始化宏（EGW_EXPORT）
- **未实现**：syslog 封装、`--daemon` 守护进程支持
- **依赖**：无（被所有层依赖）
...
M1（第 1-6 周）涉及 `app` 和 `core` 层的实现。`protocol`、`connectors` 在本阶段提供目录结构但不提供实现代码。各层在 M2 起逐步填充。
