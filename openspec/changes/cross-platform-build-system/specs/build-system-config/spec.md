...
#### Scenario: submodule 检查

- **WHEN** `build()` 执行
- **THEN** 检查 submodule 文件是否存在
- **AND** 缺失时自动 `git submodule update --init`
...
第三方依赖 SHALL 通过 git submodule 管理。

#### Scenario: 依赖添加

- **WHEN** 项目需要新的第三方库
- **THEN** 使用 `git submodule add` 添加到 `third-party/` 目录
- **AND** 锁定到指定 tag 或 commit

#### Scenario: 自动初始化

- **WHEN** `build()` 执行
- **THEN** 检查 submodule 是否已拉取
- **AND** 缺失时自动执行 `git submodule update --init`

#### Scenario: 新人克隆

- **WHEN** 开发者首次克隆仓库
- **THEN** 使用 `git clone --recursive` 一次性拉取全部 submodule
- **AND** submodule 版本与主仓库锁定一致

#### Scenario: 构建集成

- **WHEN** cmake 配置
- **THEN** 通过 `add_subdirectory(third-party/...)` 引入 submodule 源码
